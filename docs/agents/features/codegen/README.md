...
Resource groups are always entities (Entity or FSM). There is no standalone
`ResourceGroup` derive. The `#[resource_group]` and `#[resource_group(root)]`
outer attributes are detected by the `Entity` and `Fsm` derives.

**Eventless resource group** — an entity struct with no `#[event]` fields gets
an implicit declaration event generated by the derive:
...
**Entity with resource group and user events:**
...
**FSM as resource group** — non-root resource group FSMs must annotate a field
on their entry state with `#[parent_group]` to provide the parent resource
group UUID. This is enforced at compile time.
...
- Derive macros for model definition (State, Fsm, Entity)
