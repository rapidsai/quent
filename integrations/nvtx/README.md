...
Injection subscribes both NVTX ASCII surfaces:

- **Domain-scoped (CORE2)** — mark, range start/end/push/pop, domain /
  register-string / name-category / resource.
- **Classic default-domain (CORE)** — `nvtxMarkA`/`Ex`, `nvtxRangePushA`/`Ex`,
  `nvtxRangePop`, `nvtxRangeStartA`/`Ex`/`nvtxRangeEnd`, `nvtxNameCategoryA`,
  captured on the default domain (`0`) — plus OS thread naming
  (`nvtxNameOsThreadA`).

Wide-char (`*W` / Unicode) variants are subscribed with warn-once stubs:
Unicode capture is still deferred, but such a call emits a one-time diagnostic
and keeps range nesting/ids valid rather than silently becoming a no-op.
