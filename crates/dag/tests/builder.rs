...
use quent_dag::{
    DagConstraint, DagEntityBuilder, DagEventDecl, DagRole, EndpointDecl, MemberOfDecl,
};
...
fn builds_a_valid_dag_schema_fragment() {
    ...
    let vertex = DagEntityBuilder::vertex(
        ...
        MemberOfDecl::new(ident("dag"), ident("Plan")),
    )
...
fn preserves_custom_annotations_and_reference_data() {
    ...
    let vertex = DagEntityBuilder::vertex(
        ident("Operator"),
        DagEventDecl::new(ident("declared")),
        MemberOfDecl::new(ident("owner"), ident("Plan"))
