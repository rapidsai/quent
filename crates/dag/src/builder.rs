...
impl MemberOfDecl {
    /// Create a `member-of` reference named `field` targeting `dag`.
    ...
    /// Set annotations on the `member-of` field.
...
impl DagEntityBuilder {
    ...
    pub fn vertex(path: impl Into<Path>, event: DagEventDecl, member_of: MemberOfDecl) -> Self {
        Self {
            path: path.into(),
            events: Vec::new(),
            annotations: Annotations::default(),
            role: DagRole::Vertex,
            declaration: Declaration::Vertex { event, member_of },
        }
    }
    ...
    pub fn build(self) -> Result<Entity, BuilderError> {
        let declaration = match self.declaration {
            ...
            Declaration::Vertex { event, member_of } => {
                Some(build_declaration_event(event, [member_of.build()?])?)
            }
            Declaration::Edge {
                event,
                member_of,
