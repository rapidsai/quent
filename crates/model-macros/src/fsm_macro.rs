...
//! ModelComponent — invoking hidden state callback macros for flat-arg methods.
...
pub fn expand(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    let _entry_type = &entry_state.ty;
    let entry_callback = format_ident!("__quent_state_{}", entry_alias);

    // Transition methods — one per non-entry state + the entry state for re-entry
    let transition_callbacks: Vec<TokenStream> = input
        .states
        .iter()
        .map(|s| {
            let callback = format_ident!("__quent_state_{}", s.alias);
            quote! { #callback!(transition_method pub #transition_enum); }
        })
        .collect();
    ...
    let output = quote! {
            ...
            E: From<#event_type> #serde_bound + Send + 'static,
        {
            ...
            // Flat-arg transition methods generated by state callback macros
            ...
            E: From<#event_type> #serde_bound + Send + 'static,
        {
            ...
            // Flat-arg entry constructor generated by state callback macro
            #entry_callback!(entry_method pub #handle_name #transition_enum);
        }
