...
fn emit_context(model: &ModelBuilder, out: &mut String) {
    out.push_str("\nclass Context:\n");
    out.push_str("    def __init__(self, options: ExporterOptions | None = ...) -> None: ...\n");
...
pub fn emit(model: &ModelBuilder, options: &PyO3Options) -> Vec<GeneratedFile> {
    ...
    out.push_str("from os import PathLike\n");
    ...
    out.push_str("    def none() -> ExporterOptions: ...\n");
    for factory in ["ndjson", "msgpack", "postcard"] {
        out.push_str("    @staticmethod\n");
        out.push_str(&format!(
            "    def {factory}(output_dir: str | PathLike[str]) -> ExporterOptions: ...\n"
        ));
    }
    out.push_str("    @staticmethod\n");
    out.push_str("    def collector(address: str) -> ExporterOptions: ...\n");
