...
/// Env var the wrapper reads for the host/interface to bind (default `127.0.0.1`).
pub const HOST_ENV: &str = "QUENT_OPEN_HOST";
...
/// `analyzer_service_router` and serve it. Root (`<context-uuid>/` subdirs), host,
/// and port come from env so one built binary serves any artifacts.
fn main_rs(spec: &ViewerSpec) -> String {
    ...
    let (root_env, host_env, port_env) = (ROOT_ENV, HOST_ENV, PORT_ENV);
    let tokens = quote! {
...
mod tests {
    ...
    fn cargo_toml_pins_quent_and_analyzer() {
        ...
        assert!(toml.contains(
            r#"quent-simulator-analyzer = { git = "https://example.com/analyzer", rev = "analyzercommit" }"#
        ));
    }
    ...
    fn main_rs_names_viewer_and_format() {
        ...
        assert!(main.contains("QUENT_OPEN_HOST")); // host is configurable
    }
}
