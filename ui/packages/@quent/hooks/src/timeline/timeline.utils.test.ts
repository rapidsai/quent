...
import { getResourceTypeName, getFsmTypeName, bulkEntryId, setOperatorOnEntry } from './timeline.utils';
...
describe('setOperatorOnEntry', () => {
  it('sets operator_id on a Resource variant', () => {
    ...
    expect((result as { Resource: { application: TaskFilter } }).Resource.application.operator_id).toBe('op-99');
  });

  it('preserves other Resource fields when setting operator_id', () => {
    const entry = makeResourceRequest('r1', 'Fsm', null);
    const result = setOperatorOnEntry(entry, 'op-1') as { Resource: typeof entry['Resource' & keyof typeof entry] };
  ...
  it('does not mutate the original Resource entry', () => {
    ...
    const original = (entry as { Resource: { application: TaskFilter } }).Resource.application.operator_id;
    setOperatorOnEntry(entry, 'op-new');
    expect((entry as { Resource: { application: TaskFilter } }).Resource.application.operator_id).toBe(original);
  });
  ...
  it('sets operator_id on a ResourceGroup variant', () => {
    ...
    expect((result as { ResourceGroup: { app_params: TaskFilter } }).ResourceGroup.app_params.operator_id).toBe('op-7');
  });

  it('preserves other ResourceGroup fields when setting operator_id', () => {
    const entry = makeGroupRequest('g1', 'GPU', 'Worker', null);
    const result = setOperatorOnEntry(entry, 'op-2') as { ResourceGroup: typeof entry['ResourceGroup' & keyof typeof entry] };
  ...
  it('does not mutate the original ResourceGroup entry', () => {
    ...
    const original = (entry as { ResourceGroup: { app_params: TaskFilter } }).ResourceGroup.app_params.operator_id;
    setOperatorOnEntry(entry, 'op-new');
    expect((entry as { ResourceGroup: { app_params: TaskFilter } }).ResourceGroup.app_params.operator_id).toBe(original);
  });
});
