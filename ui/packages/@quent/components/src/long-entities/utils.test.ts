...
import { getDeterministicColor, type FiniteStateMachine, type FsmTransition } from '@quent/utils';
...
describe('buildLongEntityEntries', () => {
  it('returns [] for no items', () => {
    expect(buildLongEntityEntries([], colorState)).toEqual([]);
  });
  ...
  it('builds one segment per consecutive transition pair', () => {
    ...
    const [entry] = buildLongEntityEntries([fsm], colorState);
  ...
  it('keeps only states used on a filtered resource', () => {
    ...
    const [entry] = buildLongEntityEntries([fsm], colorState, new Set(['resource-1']));
  ...
  it('drops single-transition FSMs (no state span)', () => {
    ...
    expect(buildLongEntityEntries([fsm], colorState)).toEqual([]);
  });
  ...
  it('uses instance_name for the label, falling back to id', () => {
    ...
    const [n, a] = buildLongEntityEntries([named, anon], colorState);
  ...
  it('stacks non-overlapping entities onto the same row', () => {
    ...
    const entries = buildLongEntityEntries([a, b], colorState);
...
describe('getLongEntitySegmentsAtTimestamp', () => {
  it('returns every entity and its active state', () => {
    ...
    const entries = buildLongEntityEntries([first, second], colorState);
  ...
  it('uses half-open state boundaries', () => {
    ...
    const entries = buildLongEntityEntries([fsm], colorState);
