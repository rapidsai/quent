...
<svelte:options customElement={{ tag: 'quent-record-details', shadow: 'none' }} />
...
<script lang="ts">
  ...
  import RecordsSection from './details/RecordsSection.svelte';
  ...
  let records = $derived(
    schema?.records
      .map(([, record]) => record)
      .filter((record) => !path || samePath(record.path, path)) ?? [],
  );
...
<div data-quent-component="record-details">
  {#if !schema}
    <div class="quent-schema-viewer__empty" data-quent-role="empty">
      No schema selected.
    </div>
  {:else}
    <RecordsSection {records} {selection} {classes} onSelect={emitSelection} />
  {/if}
</div>
